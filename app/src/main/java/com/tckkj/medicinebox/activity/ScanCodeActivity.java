...
import com.tckkj.medicinebox.zxing.camera.CameraManager;
import com.tckkj.medicinebox.zxing.decoding.CaptureActivityHandler;
import com.tckkj.medicinebox.zxing.decoding.InactivityTimer;
import com.tckkj.medicinebox.zxing.view.ViewfinderView;
...
//        inactivityTimer = new InactivityTimer(getActivity());
        ...
        title.setText(R.string.scan_code);
...
//            }
     ...
     * App12/根据药品条码值获取药品名字 >
     * */
    private void getDrugName(final String drugBarcode){
        if (NetUtil.isNetWorking(this)){
            ThreadPoolManager.getInstance().getNetThreadPool().execute(new Runnable() {
                @Override
                public void run() {
                    httpInterface.getDrugName(drugBarcode, new MApiResultCallback() {
                        @Override
